...
if(isset($_REQUEST['btn_cp']))
{
	...
	{
		if($result =(executeQuery("SELECT `userid`, `login_name`, `password` FROM `student` WHERE 
		userid = '".$_SESSION['studId']."'")))
		{
			if(mysqli_num_rows($result) > 0)
			{
				...
				{
					if($r['login_name'] == $_REQUEST['txt_login_name'])
					{
						if($r['password'] == $_REQUEST['txt_currp'])
						{
							$result2 = (executeQuery("UPDATE `student` SET `password`='".$_REQUEST['txt_cp']."'  WHERE 
							userid =".$_SESSION['studId']));
