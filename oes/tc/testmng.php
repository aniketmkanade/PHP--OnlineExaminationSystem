...
<?php include_once('header.php');?>            <form name="testmng" action="testmng.php" method="post">
                <div class="menubar">
...
<?php
        ...
        $result = executeQuery("select t.totalquestions,t.duration,t.testid,t.testname,t.testdesc,t.subid,s.subname,DECODE(t.testcode,'".$pass_enc."') as tcode,DATE_FORMAT(t.testfrom,'%Y-%m-%d') as testfrom,DATE_FORMAT(t.testto,'%Y-%m-%d') as testto from test as t,subject as s where t.subid=s.subid and t.testname='" . htmlspecialchars($_REQUEST['edit'], ENT_QUOTES) . "' and t.tcid=" . $_SESSION['tcid'] . ";");
...
<?php
                                ...
                                $result = executeQuery("select t.testid,t.testname,t.testdesc,s.subname,DECODE(t.testcode,'".$pass_enc."') as tcode,DATE_FORMAT(t.testfrom,'%d-%M-%Y') as testfrom,DATE_FORMAT(t.testto,'%d-%M-%Y %H:%i:%s %p') as testto from test as t,subject as s where t.subid=s.subid and t.tcid=" . $_SESSION['tcid'] . " order by t.testdate desc,t.testtime desc;");
                                if (mysqli_num_rows($result) == 0) {
